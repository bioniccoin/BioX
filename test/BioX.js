// Specifically request an abstraction for BioX
var BioX = artifacts.require("BioX");
var testContract = artifacts.require("Test");
var BigNumber = require('bignumber.js');
var math = require('mathjs');
var core = require('./core');

var totalTokens = new BigNumber(15000000000).pow(8);

contract('BioX', function (accounts) {
  /* constructor */
  it("should put 15000000000 BioX in the first account", function () {
    return BioX.deployed().then(function (instance) {
      return instance.balanceOf.call(accounts[0]);
    }).then(function (balance) {
      var totalSupply = new BigNumber(15000000000).mul(new BigNumber(10).pow(8));
      assert.equal(balance.valueOf(), totalSupply.valueOf(),
          "15000000000 BioX wasn't in the first account");
    });
  });
  /* transfer */
  it("should send coin correctly", function () {
    ...
    return BioX.deployed().then(function (instance) {
      BioX = instance;
      return BioX.balanceOf.call(account_one);
    }).then(function (balance) {
      account_one_starting_balance = balance.toNumber();
      return BioX.balanceOf.call(account_two);
    }).then(function (balance) {
      account_two_starting_balance = balance.toNumber();
      return BioX.transfer(account_two, amount, {from: account_one});
    }).then(function () {
      return BioX.balanceOf.call(account_one);
    }).then(function (balance) {
  ...
  it("should send 10 tokens from contract to address", function () {
    ...
    return BioX.deployed().then(function (instance) {
      ...
      return BioX.transferFrom(accounts[0], account_one, 10,
          {from: accounts[0]});
    }).then(function () {
      return BioX.balanceOf.call(account_one);
    }).then(function (balance) {
      ...
      account_two_starting_balance = balance.toNumber();
      return test.testTransfer(BioX.address, account_two, amount,
          {from: accounts[0]});
    }).then(function () {
      return BioX.balanceOf.call(account_one);
    }).then(function (balance) {
  ...
  /* approve */
  it("should approve transfer of 10 BioX from first account to second account",
      function () {
        return core.makeApproveCheck(BioX, accounts[0], accounts[1], 10);
      });
  it("should disapprove transfer of 10 BioX from first account to second account",
      function () {
        return core.makeApproveCheck(BioX, accounts[0], accounts[1], 0);
      });
  /* transferFrom */
  it("should approve and send 10 BioX from first account to second account",
      function () {
        var BioX;

        return core.makeApproveCheck(BioX, accounts[0], accounts[1], 10).then(
            function (instance) {
              BioX = instance;
              return instance.transferFrom(accounts[0], accounts[1], 10,
                  {from: accounts[1]});
            }).then(function () {
          return BioX.balanceOf.call(accounts[1]);
        }).then(function (balance) {
          assert.equal(balance.toNumber(), 20,
              "Transfer Failed. user 2 have incorrect balance");
          return BioX.allowance.call(accounts[0], accounts[1]);
        }).then(function (balance) {
          assert.equal(balance.toNumber(), 0,
              "Allowance is incorrect after transfer");
        });
      });
  it("should use transferFrom to send 10 BioX from first account as sender also, to 7th account without approval.",
      function () {
        var BioX;
        return BioX.deployed().then(function (instance) {
          BioX = instance;
          ...
          return BioX.balanceOf.call(accounts[6]);
        }).then(function (balance) {
          assert.equal(balance.toNumber(), 10,
              "Transfer Failed. user 6 have incorrect balance");
        });
      });
  it("should approve unlimited and send 10 BioX from first account to second account",
      function () {
        var BioX;
        var UINT256_MAX = new BigNumber('2').pow(256).minus(1);

        return core.makeApproveCheck(BioX, accounts[0], accounts[1],
            UINT256_MAX).then(
            function (instance) {
              BioX = instance;
              return instance.transferFrom(accounts[0], accounts[1], 10,
  ...
  /* multiPartyTransfer */
  it("should execute a batch transfer of 10 BioX in 5 accounts", function () {
    var BioX;
    ...
    return BioX.deployed().then(function (instance) {
      ...
      for (var i = 0; i < 5; i++) {
        rets.push(BioX.balanceOf.call(targets[i]));
      }
  ...
  it("should execute a batch transfer of 10 BioX 5 times in 1 account",
      function () {
        var BioX;
        var values = [10, 10, 10, 10, 10];
        var expected = 90;
        ...
        return BioX.deployed().then(function (instance) {
          ...
          return BioX.balanceOf.call(targets[1]).then(function (balance) {
  ...
  it("should execute a batch approved transfer of 10 BioX in 5 accounts",
      function () {
        var BioX;
        var values = [10, 10, 10, 10, 10];
  ...
  it("should execute a batch approved transfer of 10 BioX 5 times in 1 account",
      function () {
        var BioX;
        var values = [10, 10, 10, 10, 10];
        var expected = 150;
  ...
  it("should not send tokens to 0 address", function () {
    return core.callAndCatchError(BioX, 'transfer',
  ...
  it("should not transferFrom _from 0 address", function () {
    return core.callAndCatchError(BioX, 'transferFrom',
  ...
  it("should not transferFrom send tokens to this contract", function () {
    return BioX.deployed().then(function (instance) {
      return core.makeApproveCheck(BioX, accounts[0], accounts[2],
          10).then(
          function () {
            return core.callAndCatchError(BioX, 'transferFrom',
  ...
  it("should not transferFrom send more tokens than it haves", function () {
    var val = totalTokens.add(1);
    return core.makeApproveCheck(BioX, accounts[0], accounts[2], val).then(
        function () {
          return core.callAndCatchError(BioX, 'transferFrom',
  ...
  it("should not multiPartyTransfer if address array length is greater to 255",
      function () {
        ...
        return core.callAndCatchError(BioX, 'multiPartyTransfer',
            [addrs, values, {from: accounts[0]}]).then(function (result) {
          assert.isTrue(result, 'Sent 10 BioX to 256 addresses');
        });
      });
  it("should not multiPartyTransfer if address array length is not equal to values array length",
      function () {
        ...
        return core.callAndCatchError(BioX, 'multiPartyTransfer',
            [addrs, values, {from: accounts[0]}]).then(function (result) {
          assert.isTrue(result, 'Sent one instance of 10 BioX to 2 addresses');
        });
      });
  ...
  it("should not multiPartyTransferFrom if address array length is greater to 255",
      function () {
        ...
        return core.callAndCatchError(BioX, 'multiPartyTransferFrom',
        ...
        return core.callAndCatchError(BioX, 'multiPartyTransferFrom',
            ...
            function (result) {
              assert.isTrue(result,
                  'Sent one instance of 10 BioX to 2 addresses');
            });
      });
  /* approve must fail checks */
  it("should not approve transfer of 10 BioX 0 spender",
      function () {
        return core.callAndCatchError(BioX, 'approve',
            [0, 10, {from: accounts[0]}]).then(function (result) {
          assert.isTrue(result, 'Approved 10 BioX to 0 spender');
        });
      });
  ...
  it("should not receive ether",
      function () {
        ...
        return BioX.deployed().then(function (instance) {
          testInst = instance;
          return BioX.web3.eth.sendTransaction({
            from: accounts[0],
            to: instance.address,
            value: BioX.web3.toWei(1, "ether")
          }, function (error) {
            msg = error.message;
            return BioX.web3.eth.getBalance(testInst.address,
                function (error, balance) {
                  assert.equal(balance.toNumber(), 0, "Contract received "
                      + BioX.web3.fromWei(balance) + " ether");
